...
class VcfNetworkInfoPanel extends ThemableMixin(PolymerElement) {
  ...
  _createComponent() {
    ...
    if (!hasInput) {
      ...
      const inputNode = {
        ...
        x: x0 - 100,
        y: y0 + (y1 - y0) / 2,
        ...this.main._getIONodeStyles(true)
      };
    ...
    if (!hasOutput) {
      ...
      const outputNode = {
        ...
        x: x1 + 100,
        y: y0 + (y1 - y0) / 2,
        ...this.main._getIONodeStyles(false)
      };
  ...
  _getComponentNodeStyles(colorId) {
    ...
    return {
      color: {
        background: this.main._shade(0.8, color),
        border: this.main._shade(0.1, color),
        highlight: {
          background: this.main._shade(0.7, color),
          border: color
        }
      },
      margin: {
